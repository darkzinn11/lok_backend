...
func NewSellerHandler(sellerService *application.SellerService) *SellerHandler {
...
func (h *SellerHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	filters, err := parseSellerFilters(r)
	if err != nil {
		h.Error(w, err.Error(), http.StatusBadRequest)
		return
	}

	sellers, svcErr := h.sellerService.List(r.Context(), claims.UserID, filters)
	if svcErr != nil {
		h.handleServiceError(w, svcErr)
		return
	}

	h.Success(w, sellers, http.StatusOK)
}
...
func (h *SellerHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	seller, svcErr := h.sellerService.GetByID(r.Context(), claims.UserID, sellerID)
	if svcErr != nil {
		h.handleServiceError(w, svcErr)
		return
	}

	h.Success(w, seller, http.StatusOK)
}
...
func (h *SellerHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	result, svcErr := h.sellerService.Create(r.Context(), claims.UserID, application.CreateSellerInput{
		Name:      req.Name,
		Email:     req.Email,
		Phone:     req.Phone,
		CPF:       req.CPF,
...
func (h *SellerHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	seller, svcErr := h.sellerService.Update(r.Context(), claims.UserID, sellerID, application.UpdateSellerInput{
		Name:      req.Name,
		Email:     req.Email,
		Phone:     req.Phone,
		CPF:       req.CPF,
...
func (h *SellerHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if svcErr := h.sellerService.Delete(r.Context(), claims.UserID, sellerID); svcErr != nil {
		h.handleServiceError(w, svcErr)
		return
	}
...
func parseSellerFilters(r *http.Request) (domain.SellerFilters, error) {
